...
class BuscaLargura:
    ...
    def __init__(self, problema):
        ...
        self.fronteira = deque([problema.inicial])  # Fila para BFS
    ...
    def passo_busca(self):
        ...
        no = self.fronteira.popleft()  # BFS: pega o primeiro nó da fila
        ...
        for filho in no.filhos(self.problema):
            if filho.estado not in self.visitados and not any(n.estado == filho.estado for n in self.fronteira):
                self.fronteira.append(filho)  # BFS adiciona ao final da fila
...
# Estados e suas ações (mesmo do exemplo anterior)
estados_romenia = [
